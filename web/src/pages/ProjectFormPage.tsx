...
// Derive stable, unique keys from labels so the person building the form never has to think about keys.
      ...
      .then((p) => {
        ...
        try {
          ...
          if (Array.isArray(fields)) {
            setRows(fields.map((f) => ({ label: f.label ?? f.key, type: f.type, required: Boolean(f.required) })))
          }
        } catch {
          /* leave empty */
        }
  ...
  const save = async () => {
    ...
    try {
      const body = JSON.stringify({
        ...
        formSchema: JSON.stringify(schemaPreview),
      })
      if (editing) {
        await api(`/api/v1/projects/${id}`, { method: 'PUT', body })
        navigate(`/projects/${id}`)
      } else {
        const created = await api<ProjectResponse>('/api/v1/projects', { method: 'POST', body })
      ...
      <div className="card">
        ...
        {rows.map((row, i) => (
          <div
            key={i}
            style={{
              display: 'grid', gridTemplateColumns: '18px 1fr 116px auto 32px', gap: 8,
              alignItems: 'center', padding: '8px 0', borderTop: i ? '1px solid var(--border)' : 'none',
            }}
          >
            ...
            <input
              ...
              placeholder="Field label (e.g. Condition)"
            />
            ...
            <label style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-2)' }}>
              <input
                type="checkbox"
